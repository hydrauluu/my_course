...
В Python **всё представлено объектами** и отношениями между объектами. Число, строка, список, функция, класс, модуль, `None` — всё без исключения. Это не метафора и не упрощение — это буквально то, как устроен язык на уровне интерпретатора.
...
Переменная в Python — это **имя, которое указывает на объект**. Не ячейка памяти с копией значения, а именно ссылка, в качестве аналогии можно представить переменную как ярлык над объектом.

Оператор присваивания `=` не копирует объект. Он связывает имя с объектом.

```python
x = 42     # имя x связывается с объектом 42
           # или же ярлык х указывать на объект 42

y = x      # имя y связывается с тем же объектом
           # ярлык y указывает на тот же объект 42
...
Каждое исключение несёт конкретный смысл. Читайте их — они говорят именно то, что произошло. Самые нужные нам это `ValueError` и `AttributeError`.
...
**2.** Намеренно вызовите `ValueError` и `AttributeError`. А так же дополнительно вызовите еще несколько видов исключений(от 3-4 штук). Используйте исключения разных классов из иерархии выше.

**3.** Объясните код приведенный ниже. Объяснения напишите в виде комментариев, что происходит в каждой из строчек.

```python
lst = [1, 2, 3]
print("=== СПИСОК ===")
print(f"До .append():  id = {id(lst)},  значение = {lst}")
 
lst.append(4)
print(f"После .append(): id = {id(lst)},  значение = {lst}")

print()

s = "hello"
print("=== СТРОКА ===")
print(f"До конкатенации:    id = {id(s)},  значение = '{s}'")

s = s + " world"
print(f"После конкатенации: id = {id(s)},  значение = '{s}'")
